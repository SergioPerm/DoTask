//
//  SettingsTasksShowDoneToggle.swift
//  DoTask
//
...
class SettingsTasksShowDoneSwitch: UIView {

    var changeSettingHandler: (() -> Void)?
    var showDone: Bool {
        didSet {
            switchControl.isOn = showDone
        }
    }
    ...
    let title: LocalizableLabel = {
        ...
        label.localizableString = LocalizableStringResource(stringResource: R.string.localizable.settings_SHOW)
...
extension SettingsTasksShowDoneSwitch {
    private func setup() {
        addSubview(title)
